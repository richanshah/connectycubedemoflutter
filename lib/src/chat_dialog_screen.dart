...
class ChatScreenState extends State<ChatScreen> {
  ...
  StreamSubscription<LastActivitySubscriptionEvent>? lastActivitySubscription;
  ...
  _initChatListeners() {
    ...
    CubeChatConnection.instance.lastActivityManager
        ?.subscribeToUserLastActivityStatus(_cubeUser.id!, callback: (seconds) {
      log("lastActivity: userId = ${_cubeUser.id}, seconds = ${seconds}");
    });
    lastActivitySubscription = CubeChatConnection
        .instance.lastActivityManager?.lastActivityStream
        .listen(onLatActivityEvent);
  }

  void onLatActivityEvent(LastActivitySubscriptionEvent lastActivityEvent) {
    log("lastActivityEvent: userId = ${lastActivityEvent.userId}, seconds = ${lastActivityEvent.seconds}");
    if (lastActivityEvent.seconds < 300) {
      // The user is online
      log('User is online' + lastActivityEvent.seconds.toString());
    } else {
      // The user is offline
      log('User is offline' + lastActivityEvent.seconds.toString());
    }
  }
